...
import Avatar,{pixelCharacters} from './Avatar'

export default function AvatarCreator({avatar,setAvatar,onSave}){
  const valid=avatar.name.trim().length>=2,list=pixelCharacters[avatar.gender==='girl'?'girl':'boy']
  ...
  const gender=(value)=>setAvatar({...avatar,gender:value,character:value==='girl'?4:0})
  return <motion.main className="screen onboarding" initial={{opacity:0}} animate={{opacity:1}}>
    ...
    <form className="creator-card polished pixel-creator" onSubmit={(e)=>{e.preventDefault();if(valid)onSave()}}>
      ...
      <div className="gender-tabs"><button type="button" className={avatar.gender==='boy'?'active':''} onClick={()=>gender('boy')}>Menino</button><button type="button" className={avatar.gender==='girl'?'active':''} onClick={()=>gender('girl')}>Menina</button></div>
